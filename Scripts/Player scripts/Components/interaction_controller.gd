...
func collision():
	if raycast.is_colliding():
		...
		if collider is Interactable_RB or collider is Interactable_SB:
			prompting()

			if collider.interaction_enabled:
...
func debug():
	if raycast.is_colliding():
		...
		if collider is Interactable_RB or collider is Interactable_SB and ! trying_to_hold:
